...
// Since simple inorder traversal cannot uniquely identify a tree, we use symbols like ‘(‘ and ‘)’ to represent NULL nodes. 
// We pass an Unordered Map in C++ as an argument to the helper function which recursively 
// calculates inorder string and increases its count in map. If any string gets repeated, then it will imply duplication 
// of the subtree rooted 
// at that node so push that node in the Final result 
